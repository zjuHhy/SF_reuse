...
class VAETRTWrapper():
    def __init__(self):
        ...
        test_input = torch.zeros(1, 16, 1, 60, 104).cuda().bfloat16()
        is_first_frame = torch.tensor(1.0).cuda().bfloat16()
        test_cache_inputs = [c.cuda().bfloat16() for c in ZERO_VAE_CACHE]
