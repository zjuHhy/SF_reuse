...
dummy_input = torch.randn(1, 1, 16, 60, 104).bfloat16().cuda()
is_first_frame = torch.tensor([1.0], device="cuda", dtype=torch.float16)
dummy_cache_input = [
    torch.randn(*s.shape).bfloat16().cuda() if isinstance(s, torch.Tensor) else s
...
# 2️⃣  Export → ONNX
# ─────────────────────────────────────────────────────────
model = VAEDecoderWrapperSingle().bfloat16().cuda().eval()
...
model = model.bfloat16().cuda().eval()                          # only batch dim dynamic
