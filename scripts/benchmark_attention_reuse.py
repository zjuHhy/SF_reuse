...
Usage:
    python scripts/benchmark_attention_reuse.py --reuse_ratios 0.0 0.5 0.8 0.95 --num_runs 100
"""
...
class CUDATimer:
    ...
    def end(self):
        ...
        if torch.cuda.is_available():
            ...
            ms = self._start_evt.elapsed_time(end_evt)
        else:
            ms = (time.perf_counter() - self._start_cpu) * 1000.0
        self.times.setdefault(self._name, []).append(ms)
        self._name = None
...
class BenchmarkCausalWanSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if f > 1 and step_idx > warmup_steps:
            ...
            q_hilbert = q_spatial[:, :, self.hilbert_to_raster, :]
            sub_C = max(C // 4, 1)
            ...
            for i in range(1, f):
                ...
                if search_radius == 0:
                    ...
                    sims_list = []
                    for dx in range(search_radius * 2 + 1):
                        q_prev_shifted = q_prev_pad[:, :, dx:dx + N]
        ...
        if reuse_mask.any():
            if b == 1:
                ...
                x_out = torch.empty_like(roped_query)
...
def generate_reuse_mask(
    ...
    """
    Generate a synthetic reuse mask.
    Frame 0 is never reused; reuse is applied among frames 1..num_frames-1.
    ratio: fraction of tokens among frames 1..num_frames-1 that are marked reused.
    """
    ...
    if pattern == "block":
        ...
        for blk in chosen:
            ...
            # map flat position (within reusable region) to actual position
        ...
        actual_indices = reusable_indices + N  # skip frame 0
...
def build_kv_cache(batch_size, max_len, n_heads, head_dim, device):
    return {
        ...
        "step_idx": torch.tensor(10, device=device),  # skip warmup
...
def benchmark(
    ...
    for _ in range(num_runs):
        # Reset kv cache indices so each run sees identical cache state
        kv_cache["global_end_index"].fill_(current_start)
        kv_cache["local_end_index"].fill_(current_start)
        kv_cache["step_idx"] = torch.tensor(10, device=device)
        kv_cache["last_start"] = torch.tensor(-1, device=device)
...
def main():
    ...
    parser.add_argument(
        ...
        help="Skip internal reuse-mask computation and inject synthetic mask directly. "
             "If False, internal logic still runs but its result may be overwritten by injection.",
    )
    ...
    # Synthetic input
    x = torch.randn(b, total_len, dim, device=device)
    ...
    # Freqs: shape [1024, head_dim // 2]
    # causal_rope_apply splits freqs into 3 parts:
    # [c - 2*(c//3), c//3, c//3]
    c_half = head_dim // 2
    freqs = torch.randn(1024, c_half, device=device,dtype=torch.bfloat16)

    # KV cache: pre-allocate enough for history + current
    cache_capacity = max(32768, total_len * 4)
    kv_cache = build_kv_cache(b, cache_capacity, num_heads, head_dim, device)

    # Pre-fill cache so the model sees history (simulate non-first chunk)
    current_start = 2 * N  # pretend first 2 frames are already cached
    kv_cache["global_end_index"].fill_(current_start)
    kv_cache["local_end_index"].fill_(current_start)
    # Fill cache with random data up to current_start
    ...
    # Print results
    # ------------------------------------------------------------------
    stages = ["rope", "kv_cache_update", "reuse_logic", "attention", "compensation"]
    header = f"{'Stage':<20} " + " ".join([f"{lbl:<18}" for lbl, _ in results])
    print(header)
    print("-" * len(header))
    for stage in stages:
        row = f"{stage:<20} "
        for lbl, summary in results:
            if stage in summary:
                mean = summary[stage]["mean_ms"]
                std = summary[stage]["std_ms"]
                row += f"{mean:6.2f}±{std:5.2f} ms   "
            else:
                row += f"{'N/A':<18} "
        print(row)

    print("-" * len(header))
    row_total = f"{'total_forward':<20} "
    for lbl, summary in results:
        total = 0.0
        for stage in stages:
            if stage in summary:
                total += summary[stage]["mean_ms"]
        row_total += f"{total:6.2f} ms         "
    print(row_total)

    # Extra: compute achieved token reduction in attention
    print("\n" + "-" * 80)
    print("Attention token reduction analysis (relative to full cache):")
    cache_len = current_start + total_len
    for ratio in args.reuse_ratios:
        if ratio > 0:
