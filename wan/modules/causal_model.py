        ...
        if kv_cache is None:
            ...
            if is_tf:
                ...
                if reuse_mask.any():
                    ...
                    # ==========================================
                    # 【新增】计算完整的 Attention 作为基准参考
                    # ==========================================
                    with torch.no_grad(): # 如果不需要计算梯度，可以用 no_grad 节省显存
                        # with timer("attn_time_full_ref"):
                            x_full_ref = attention(roped_query, k_cache_sliced, v_cache_sliced)
                    # ==========================================
                    ...
                    # ==========================================
                    # 【新增】计算并打印复用后 Attention 的 MSE 与最大误差
                    # ==========================================
                    mse_diff = F.mse_loss(x, x_full_ref)
                    max_diff = torch.abs(x - x_full_ref).max()
                    print(f"步数 {step_idx} - 复用 vs 完整计算 -> MSE: {mse_diff.item():.6e}, Max Diff: {max_diff.item():.6e}")
                    # 如果为了不影响后续梯度传播并使用完整特征作为后续输出（仅测试误差），您可以取消下面这行的注释
                    # x = x_full_ref 
                    # ==========================================
