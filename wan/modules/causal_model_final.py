...
def get_hilbert_mapping_indices(H: int, W: int, device='cpu'):
    """生成支持非正方形 (如 30x52) 的 Hilbert 与 Raster 双向映射索引"""
    # 找到能包围 H 和 W 的最小 2 的幂 (对于 30x52，n=64)
    n = 1
    while n < max(H, W):
        n *= 2
    ...
    def forward(
        ...
        if kv_cache is None:
            ...
            if is_tf:
                ...
                # ====================================================
                # [Self-Forcing] 去噪热身机制 (Warm-up) & 1D 运动补偿
                # ====================================================
                ...
                if f > 1 and step_idx > warmup_steps:
                    ...
                    # 使用写死的 1560 映射索引进行 1D Hilbert 重排
                    ...
                    for i in range(1, f):
                        ...
                        # 重塑为 Block 视图: [B, num_blocks, block_size]
                        mask_blocks = token_mask.view(b, num_blocks, block_size)
                        
                        # 策略 A 
                        # 只有 Block 内所有 Token 都满足复用条件，才复用整个 Block
                        # block_decision = mask_blocks.all(dim=-1, keepdim=True) 
                        
                        # 策略 B: 多数表决
                        # 如果 Block 内有 80% 的 Token 满足条件，就强制复用整个 Block
                        threshold_ratio = 0.8
                        block_decision = (mask_blocks.float().mean(dim=-1, keepdim=True) >= threshold_ratio)
                        
                        # 将 Block 决定广播回 Token 级别: [B, num_blocks, block_size] -> [B, N]
                ...
                if reuse_mask.any():
                    ...
                    for i in range(1, f):
                        ...
                        # 上一帧的特征图
                        x_prev_raster = x[:, prev_start:prev_end].view(b, N, C)
                        
                        # 极速 Gather 补偿
                        # 把上一帧特征映射到 Hilbert 空间
                        # 我们在计算运动偏移量 best_indices_hilbert 时，是在 Hilbert 一维空间里找的坐标
                        # 如果不在同一个空间，坐标就乱套了。
                        ...
                        x[:, start_idx:end_idx] = torch.where(mask_expanded, best_x_raster, x[:, start_idx:end_idx])
                else:
                    with timer("attn_time"):
